...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    fluidRow(
      box(width = 5, 
          strong("Overview"), br(),
          br(),
          "This dashboard is intended to provide a set of potentially useful indicators related to the accuracy of officially reported data on the spread of the COVID-19 virus. Please note that the authors of the page", em(strong("are not medical professionals,")), "and that the data provided are based on mathematical analysis of data from other sources.", em(strong("The data provided here should not be used to replace or supercede any authoritative information developed from any official or medical source.")), br(),
          br(),
          "The purpose of the dashboard is to provide a different view of existing, available data on the COVID-19 infection. At the time of the creation of this dashboard (March 29, 2020), the vast majority of public reporting on the COVID-19 pandemic in the United States has focused primarily on two values: (1) the number of confirmed cases; and (2) the number of confirmed deaths (which are displayed here for the convenience of the user). However, given the initial and ongoing limitations on testing for COVID-19 within the United States, together with the likely presence of large numbers of asymptomatic patients, it is clear that actual infection rates greatly exceed the number of confirmed cases.", br(),
          br(),
      ...
      box(width = 7,
          ...
          br(),
          strong("Confirmed Case Doubling Period:"), "The number of days it takes for the number of confirmed cases to double. Given the currently documented reproductive rate of SARS-Cov-19, epidemiological models suggest an expected doubling period of approximately 6 days in the absence of any mitigation or suppression efforts. Since most areas have been undertaking some level of mitigation/suppression for several weeks, the default expected doubling period has been set at 12 days in an effort to reflect a reasonable reduction in transmission rates (users may also select a different doubling period if desired). Here,", em(strong("the warning sign is a doubling period that is faster than the expected value"))," (above the line on the graph, which has a reversed Y-axis). Where the local doubling period is faster than expected, it could mean that: (a) testing is being restricted or targeted primarily at patients with a higher probability of infection (such as severe cases); or (b) the population being tested otherwise already has a high rate of infection; in either case, the number of actual cases is likely to be much higher than the number of confirmed cases. A doubling rate lower than the expected value suggests that the number of confirmed cases may be closer to the actual number of cases present. The trend line indicates the direction of change (i.e. a trend towards the expected value indicates an improvement in the doubling period). A downward trend could reflect a more robust testing effort and/or beneficial effects of mitigation/suppression measures in limiting the growth in new cases. ", br(),
          br(),
          strong("Apparent Case Fatality Rate:"), "The ratio of deaths attributed to COVID-19 to total confirmed cases. Current estimates of the case fatality ratio (CFR) for COVID-19 infections range widely but have been estimated by various experts at around 0.9 – 1.2% of infected patients, with an average period from infection-to-death at around 31 days. Because infections tend to spread during this intervening period, during the early phase of an epidemic the ratio of confirmed deaths to confirmed cases would normally be expected to be lower than the actual CFR, converging on the expected CFR over time. Here,", em(strong("the warning sign is a ratio higher than the expected CFR.")), "If more patients are dying as compared to total confirmed cases than would otherwise be expected, this could mean that the infection rate is substantially higher than the confirmed rate, since the confirmed case count includes patients that were infected 31 days ago plus those infected since that time. This could also signal that the local CFR is higher, due to the presence of relatively more vulnerable populations, limitations on medical care, or similar factors. The trend line indicates the direction of change (i.e. a trend towards the expected value indicates an improvement in the apparent case fatality ratio).", br(),
          br(),
          strong("Deaths as Percentage of Population:")," The number of deaths as a percentage of state population, as compared to the national mean value. Here,", em(strong("the warning sign is a death rate in the population that exceeds the national mean by at least one standard deviation.")), "Because of the substantial lag time that typically exists between initial infection and the death of an infected patient, a higher fraction of deaths likely indicates that a higher relative percentage of the local population was infected several weeks ago in comparison to other areas, and that infections may thus have spread more widely in the intervening period in the absence of effective suppression. A relatively higher CFR could also reflect the selective impact of the disease on a relatively more vulnerable population (e.g. nursing homes or other high-risk facilities) or the inadequacy of local medical care.", br(), 
          br(), 
  ...
  tabPanel(
    ...
    fluidRow(
      column(width = 8,
             box(width = 12,
                 "The creators of this dashboard are non-medical professionals with experience in data visualization and scenario planning. Please note that", em(strong("we do not have any medical expertise or formal training in epidemiology.")), "The information provided here is based solely on a mathematical analysis of data from other sources, and it", em(strong("should not be used to replace or supercede any authoritative information from any official or medical source.")), br(),
                 br(),
